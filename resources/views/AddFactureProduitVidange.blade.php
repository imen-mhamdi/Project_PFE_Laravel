<html>
 ...
 <body>
  ...
  <div class="form-group">
    <label for="code_frs">Code Frs</label>
    <select class="form-control" 
    name="code_frs"
    id="code_frs"
    >
    @foreach($fournisseur as $forn)
      <option value="{{$forn->code_frs}}">{{$forn->code_frs}}</option>
   ...
   <div class="table-responsive">
               ...
               <thead>
                ...
                <th width="0.5%"></th>
                    ...
                    <th width="20%">Qte Produit Vidange</th>
...
$(document).ready(function(){
 ...
 function dynamic_field(number)
 {
  ...
  html = '<tr  class="row">';
        html += '<td><select id="code_pie[]" name="code_pie[]" class="form-control" >@foreach($codePiece as $forn)<option value="{{$forn->Code_prd_vid}}">{{$forn->Code_prd_vid	}}</option>@endforeach</select></td>';
...
// function recalculant le prix d'un ligne et retournant le prix
 ...
 $('#dynamic_form').on('submit', function(event){
        ...
        console.log("code facture",code_facture)
        var code_frs = $('#code_frs').find(":selected").val()
        $.ajax({
          url:`/facture_vidange/${code_facture}/${code_frs}`,
            method:'post',
            data:$(this).serialize(),
            dataType:'json',
            beforeSend:function(){
